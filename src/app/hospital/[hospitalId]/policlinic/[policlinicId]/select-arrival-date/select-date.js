...
function PopUp({ dayName, date, year, month, onRemove }) {
  ...
  const newPathname = pathname.replace("/select-arrival-date","");
  ...
  return (
    <article className="flex flex-col items-center border-2 z-20 bg-white py-6 px-12 rounded-t-xl fixed bottom-0 right-0 left-0">
      ...
      <footer className="flex flex-col items-center justify-center w-full">
        <Link href={newPathname + "/register-form-step-1"} className="w-full">
...
export function Card({ date, dayName, month, year, arrivalDate, onClick }) {
  ...
  const isWeekend = ["Sabtu", "Minggu"].includes(dayName);
  let addStyle;
  ...
  return (
    <button disabled={isWeekend} onClick={handleClick}>
      <article
        className={
          addStyle +
      ...
      >
        ...
        <p className="font-light text-sm">
          {isWeekend ? "Libur" : "Total Pendaftar : 30 orang"}
        </p>
      </article>
    </button>
  );
